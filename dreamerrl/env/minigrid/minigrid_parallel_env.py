...
def make_env(env_cfg: EnvironmentConfig, idx: int) -> Callable[[], gym.Env]:
    def thunk():
        # Reset global NumPy RNG BEFORE construction (for deterministic map/layout)
...
class MinigridParallelEnv:
    """
    True parallel Minigrid environments using AsyncVectorEnv.
    Each environment runs in its own process.

    Observations are flattened into vectors suitable for Dreamer’s encoder.
    """
    ...
    def __init__(self, env_cfg: EnvironmentConfig, device: torch.device):
        ...
        if not isinstance(self.single_action_space, Discrete):
            raise RuntimeError(
                f"MinigridParallelEnv only supports Discrete action spaces, got {type(self.single_action_space)}"
            )
    ...
    def action_dim(self) -> int:
        ...
        if not isinstance(space, Discrete):
            raise RuntimeError(f"MiniGridParallelEnv only supports Discrete action spaces, got {type(space)}")
        return int(space.n)
